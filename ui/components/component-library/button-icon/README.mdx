...
Use the required `iconName` prop with `IconName` enum from `./ui/components/component-library/icon` to select icon.
...
Optional: `ButtonIconSize` from `./button-icon` object can be used instead of `Size`.
